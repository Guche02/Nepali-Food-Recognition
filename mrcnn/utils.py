...
def compute_accuracy(gt_boxes, gt_class_ids, pred_boxes, pred_class_ids, iou_threshold=0.5):
    """Compute accuracy of object detection."""
    ...
    accuracy = correct_detections / total_objects if total_objects > 0 else 0.0
    return accuracy
...
def gt_pred_lists(gt_class_ids, gt_bboxes, pred_class_ids, pred_bboxes, iou_thresh=0.5):
    ...
    this function associates the predicted classes to their gt classes using a given IoU threshold (IoU >= 0.5 for example) 
    and returns two normalized lists of len = N containing the gt and predicted classes, 
    filling the non-predicted and mis-predicted classes by the background class (index 0).
    ...
    Returns: 
        ...
        pred           : list of size N 
    """
    # Dictionary containing the state of each gt and predicted class (0: not associated to any other class, 1: associated to a class)
    ...
    # Look for objects that are not predicted (gt objects that don't exist in pred objects)
    for i, gt_class in enumerate(gt_class_ids_["gt_class_ids"]):
        if gt_class_ids_['state'][i] == 0:
            gt.append(gt_class)
            pred.append(0)
    # Look for objects that are mispredicted (pred objects that don't exist in gt objects)
    for j, pred_class in enumerate(pred_class_ids_["pred_class_ids"]):
...
def pretty_plot_confusion_matrix(df_cm, annot=True, cmap="Blues_r", fmt='.2f', fz=11,
...
def plot_confusion_matrix_from_data(y_test, predictions, columns=None, annot=True, cmap="Blues_r",
      fmt='.2f', fz=11, lw=0.5, cbar=False, figsize=[36,36], show_null_values=0, pred_val_axis='lin'):
    """
        plot confusion matrix function with y_test (actual values) and predictions (predic),
        whitout a confusion matrix yet
